...
/**
 * This class defines the structure of the 'attribute_type' table.
 *
 *
 *
 ...
 *
 */
class AttributeTypeTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'AttributeType.Model.Map.AttributeTypeTableMap';
     ...
     */
    const OM_CLASS = '\\AttributeType\\Model\\AttributeType';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'AttributeType.Model.AttributeType';
     ...
     */
    const ID = 'attribute_type.ID';

    /**
     * the column name for the SLUG field
     */
    const SLUG = 'attribute_type.SLUG';

    /**
     * the column name for the HAS_ATTRIBUTE_AV_VALUE field
     */
    const HAS_ATTRIBUTE_AV_VALUE = 'attribute_type.HAS_ATTRIBUTE_AV_VALUE';

    /**
     * the column name for the IS_MULTILINGUAL_ATTRIBUTE_AV_VALUE field
     */
    const IS_MULTILINGUAL_ATTRIBUTE_AV_VALUE = 'attribute_type.IS_MULTILINGUAL_ATTRIBUTE_AV_VALUE';

    /**
     * the column name for the PATTERN field
     */
    const PATTERN = 'attribute_type.PATTERN';

    /**
     * the column name for the CSS_CLASS field
     */
    const CSS_CLASS = 'attribute_type.CSS_CLASS';

    /**
     * the column name for the INPUT_TYPE field
     */
    const INPUT_TYPE = 'attribute_type.INPUT_TYPE';

    /**
     * the column name for the MAX field
     */
    const MAX = 'attribute_type.MAX';

    /**
     * the column name for the MIN field
     */
    const MIN = 'attribute_type.MIN';
     ...
     */
    const UPDATED_AT = 'attribute_type.UPDATED_AT';
    ...
    {
        // attributes
        $this->setName('attribute_type');
        $this->setPhpName('AttributeType');
        $this->setClassName('\\AttributeType\\Model\\AttributeType');
        $this->setPackage('AttributeType.Model');
        $this->setUseIdGenerator(true);
        // columns
        $this->addPrimaryKey('ID', 'Id', 'INTEGER', true, null, null);
        $this->addColumn('SLUG', 'Slug', 'VARCHAR', false, 50, null);
        $this->addColumn('HAS_ATTRIBUTE_AV_VALUE', 'HasAttributeAvValue', 'TINYINT', false, null, 0);
        $this->addColumn('IS_MULTILINGUAL_ATTRIBUTE_AV_VALUE', 'IsMultilingualAttributeAvValue', 'TINYINT', false, null, 0);
    ...
    {
        $this->addRelation('AttributeAttributeType', '\\AttributeType\\Model\\AttributeAttributeType', RelationMap::ONE_TO_MANY, array('id' => 'attribute_type_id', ), 'CASCADE', 'RESTRICT', 'AttributeAttributeTypes');
        $this->addRelation('AttributeTypeI18n', '\\AttributeType\\Model\\AttributeTypeI18n', RelationMap::ONE_TO_MANY, array('id' => 'id', ), 'CASCADE', null, 'AttributeTypeI18ns');
    } // buildRelations()
    ...
    /**
     * Method to invalidate the instance pool of all tables related to attribute_type     * by a foreign key with ON DELETE CASCADE
     */
    ...
    {
        return $withPrefix ? AttributeTypeTableMap::CLASS_DEFAULT : AttributeTypeTableMap::OM_CLASS;
    }
     ...
     * @return array (AttributeType object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = AttributeTypeTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = AttributeTypeTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + AttributeTypeTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = AttributeTypeTableMap::OM_CLASS;
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            AttributeTypeTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = AttributeTypeTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = AttributeTypeTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(AttributeTypeTableMap::ID);
            $criteria->addSelectColumn(AttributeTypeTableMap::SLUG);
            $criteria->addSelectColumn(AttributeTypeTableMap::HAS_ATTRIBUTE_AV_VALUE);
            $criteria->addSelectColumn(AttributeTypeTableMap::IS_MULTILINGUAL_ATTRIBUTE_AV_VALUE);
            $criteria->addSelectColumn(AttributeTypeTableMap::PATTERN);
            $criteria->addSelectColumn(AttributeTypeTableMap::CSS_CLASS);
            $criteria->addSelectColumn(AttributeTypeTableMap::INPUT_TYPE);
            ...
            $criteria->addSelectColumn($alias . '.HAS_ATTRIBUTE_AV_VALUE');
            $criteria->addSelectColumn($alias . '.IS_MULTILINGUAL_ATTRIBUTE_AV_VALUE');
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(AttributeTypeTableMap::DATABASE_NAME)->getTable(AttributeTypeTableMap::TABLE_NAME);
    }
    ...
    {
      $dbMap = Propel::getServiceContainer()->getDatabaseMap(AttributeTypeTableMap::DATABASE_NAME);
      if (!$dbMap->hasTable(AttributeTypeTableMap::TABLE_NAME)) {
        $dbMap->addTableObject(new AttributeTypeTableMap());
      }
    }

    /**
     * Performs a DELETE on the database, given a AttributeType or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or AttributeType object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \AttributeType\Model\AttributeType) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(AttributeTypeTableMap::DATABASE_NAME);
            $criteria->add(AttributeTypeTableMap::ID, (array) $values, Criteria::IN);
        }

        $query = AttributeTypeQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) { AttributeTypeTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) { AttributeTypeTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return AttributeTypeQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a AttributeType or Criteria object.
     *
     * @param mixed               $criteria Criteria or AttributeType object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from AttributeType object
        }

        if ($criteria->containsKey(AttributeTypeTableMap::ID) && $criteria->keyContainsValue(AttributeTypeTableMap::ID) ) {
...
//
AttributeTypeTableMap::buildTableMap();
